...
test_that("SVsharp works for correct input.", {
  expect_equal(SVboundsharp(BF_U = 2, prob = 0.4), "SV bound is sharp.")
  expect_equal(SVboundsharp(BF_U = 2, prob = 0.9), "Inconclusive.")
  expect_equal(SVboundsharp(BF_U = 2, prob = 0.9, AFbound = 1.5, SVbound = 2),
...
test_that("SVboundsharp throws an error for incorrect input.", {
  expect_error(SVboundsharp(BF_U = 0.5, prob = 0.8), "greater than or equal to 1")
  expect_error(SVboundsharp(BF_U = 2, prob = 2), "not between 0 and 1")
  expect_error(SVboundsharp(BF_U = 2, prob = -2), "not between 0 and 1")
})
