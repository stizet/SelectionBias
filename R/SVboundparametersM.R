#' Calculate the Smith and VanderWeele bias bound parameters for the M structure (Smith, L. H., & VanderWeele, T. J. (2019). Bounding bias due to selection.).
#'
...
#' @param pY1coef A numerical vector with two elements. The first element is the intercept in the model for the potential outcome Y(1) (as the defined by the input "probit"). The second element is the slope in the model for the potential outcome Y(1) (as defined by the input "probit").
#' @param pY0coef A numerical vector with two elements. The first element is the intercept in the model for the potential outcome Y(0) (as the defined by the input "probit"). The second element is the slope in the model for the potential outcome Y(0) (as defined by the input "probit").
...
#'
#' @return A list containing the Smith and VanderWeele bias bound parameters and an indicator if the treatment has been reversed.
#' @export
#'
#' @examples
#' pV = 0.1
...
SVboundparametersM <- function(pVprob,pUprob,pTcoef,pY1coef,pY0coef,pScoef,whichPar,probit)
{
  ...
  ### CALCULATING THE TERMS OF THE SV BOUND ###

  if(testSelBias<biasLimit)
  {
    parts = boundPartsMultSFunc(pY0,pY1,pTnew,pSmatNew,pU,pV,whichPar)
  }else{parts = boundPartsMultSFunc(pY1,pY0,pT,pSmat,pU,pV,whichPar)}

  ### END CALCULATING THE TERMS OF THE SV BOUND ###

  if(whichPar=="RD_s"|whichPar=="RR_s")
  {
    heading = c("BF_U","RR_TU|S=1","RR_UY|S=1","Reverse treatment")
    values = list(parts[1],parts[2],parts[3],as.logical(revTreat))
  }else{
    heading = c("BF_1","RR_SU|T=1","RR_UY|T=1","BF_0","RR_SU|T=0","RR_UY|T=0","Reverse treatment")
    values = list(parts[1],parts[2],parts[3],parts[4],parts[5],parts[6],as.logical(revTreat))
  }
