biasBoundMultSFunc <- function(Y1,Y0,Tr,S,U,V,parameter)
{
  # Different calculations are needed for the two different populations.
  # If the population is the subpop, do the first
  ...
  if(parameter=="RD_s"|parameter=="RR_s")
  {
    ...
    # Bounds for the risk difference and relative risk, in the selected subpopulation.
    # bound(RD_s) = max(P(Y=1|T=0,S=1)*(BF_U-1),P(Y=1|T=1,S=1)*(1-1/BF_U))
    boundRD_s = round(max(pY_T0*(BF_U-1),pY_T1*(1-1/BF_U)),digits = 14)
    boundRR_s = round(BF_U,digits = 14)

    if(parameter=="RD_s"){returnBound = boundRD_s}else{returnBound = boundRR_s}
  }else{
    ...
    # The bounds in the total population.
    # bound(RD_tot) = BF_1-P(Y=1|T=1,S=1)/BF_1+P(Y=1|T=0,S=1)*BF_0
    boundRD_tot = BF1-pY_T1/BF1+pY_T0*BF0
    # bound = BF_1*BF_0
    boundRR_tot = BF1*BF0

    if(parameter=="RD_tot"){returnBound = boundRD_tot}else{returnBound = boundRR_tot}
  }
  return(returnBound)
}
