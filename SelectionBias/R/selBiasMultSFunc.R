selBiasMultSFunc <- function(Y1,Y0,Tr,S,U,V,parameter)
{
  ...
  # Now, do the calculations for the causal parameter of interest, P(Y(1)=1|S=1) and P(Y(0)=1|S=1).
  ...
  for (iii in 1:2)
  {
    for (jjj in 1:2)
    {
      ...
      # The numerator for P(Y(1)=1|S=1): P(U=u)*P(V=v)*P(Y1=1|U=u)*P(T=t|V=v)*P(S=1|T=t,U=u,V=v).
      Y1numUB = U[jjj]*V[iii]*Y1[1,jjj]*(sProd1*Tr[1,iii]+sProd0*Tr[2,iii])

      # The denominator for P(Y(1)=1|S=1): P(U=u)*P(V=v)*P(T=t|V=v)*P(S=1|T=t,U=u,V=v).
      Y1denomUB = U[jjj]*V[iii]*(sProd1*Tr[1,iii]+sProd0*Tr[2,iii])

      # The numerator for P(Y(0)|S=1): P(U=u)*P(V=v)*P(Y0=1|U=u)*P(T=t|V=v)*P(S=1|T=t,U=u,V=v).
      Y0numUB = U[jjj]*V[iii]*Y0[1,jjj]*(sProd1*Tr[1,iii]+sProd0*Tr[2,iii])

      # The denominator for P(Y(0)|S=1): P(U=u)*P(V=v)*P(T=t|V=v)*P(S=1|T=t,U=u,V=v).
      Y0denomUB = U[jjj]*V[iii]*(sProd1*Tr[1,iii]+sProd0*Tr[2,iii])
  ...
  # Now, do the calculations for the causal parameter of interest in the total population, P(Y(1)=1) and P(Y(0)=1).
  ...
  for (jjj in 1:2)
  {
    # P(Y(1)=1): P(U=u)*P(Y1=1|U=u).
    Y1UB = U[jjj]*Y1[1,jjj]

    # P(Y(0)=1): P(U=u)*P(Y0=1|U=u).
    Y0UB = U[jjj]*Y0[1,jjj]

    # Store the values of the probabilities.
